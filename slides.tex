...
\begin{frame}{Overview}
    ...
    Impulse response estimation usually tends to be handled by vector autoregression models. Is there possibly
    a better way?
...
\begin{frame}{Local Projections (LPs)}
    ...
    Jorda (2005) proposed local projections (LPs) as an alternative method for estimating impulse
    responses. \bigskip
    ...
    \begin{equation*}
        y_{t+h} = \alpha^{(k)} + \beta^{(k)}x_t + \Gamma^{(k)}\mathbf{W}_t + u_{(k),t+h}
    \end{equation*}
...
\begin{frame}{Where does our project fit in?}
    ...
    time-series dataframe, along with a set of examples comparing the SLP method to standard VAR-based IRFs, using both simulated and real data
    \bigskip
    ...
    \begin{itemize}
        ...
        \item Turning the code into a Python package for use by other researchers
        \item Creating example Jupyter notebooks to demonstrate the method and compare it to VAR-based IRFs
